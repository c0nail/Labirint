...
//	string pathToPrefabFruit = "Prefabs/BackLab";
	...
	public void Draw(ILabirint labirint, ICreater creater)
	{
		scale = 1;
		distance = 0.95f;
		...
		Cells = creater.GetBlock (sizeFieldX);
		...
		//		Cells[i-1,j-1] = (GameObject)GameObject.Instantiate(Resources.Load(pathToPrefabFruit),positonFruit, Quaternion.identity);
				Cells[i-1,j-1].transform.position = positonFruit;
				...
				if (maxSize > maxElement) {
					...
					Cells[i-1,j-1].transform.localScale = new Vector3(baseScale,baseScale,1);
				}
			}	
		}
	}
