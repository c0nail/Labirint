...
public class Finder : IFinder {
	...
	bool Find(int ax, int ay, int bx, int by)  
	{
		...
		do {
			...
			for (int i = 0; i < sizeLab; i++)
				...
				if ( grid[i,j] == d )                        
				{
					...
					if (i-1 >= 0 && grid[i-1,j] >= 0 && (grid[i-1,j] == 1 || grid[i-1,j] == 4 || grid[i-1,j] == 5 || grid[i-1,j] == 8 ||
					...
					if (i+1 < sizeLab && grid[i+1,j] >= 0 && grid[i+1,j] < 8)
					{
						stop = false;                                                     
						grid[i+1,j] = d + 1;
					}

					if (j-1 >= 0 && grid[i,j -1] >= 0 && (grid[i,j -1] < 4 || (grid[i,j -1] > 7 && grid[i,j -1] < 12)))
					{
						stop = false;                                                 
						grid[i,j -1] = d + 1;
					}

					if (j+1 < sizeLab && grid[i,j +1] >= 0 && ( grid[i,j+1] == 2 || grid[i,j +1] == 4 || grid[i,j +1] == 6 ||
			...
			d++;
		} while ( !stop && grid[by,bx] > 0);


		if (grid[by,bx] >= 0) return false;  // путь не найден
		...
		while ( d > -1000 )
		{
			...
			{
				x -= 1;
			}else if (x+1 < sizeLab && grid[x+1,y] == d && originalGrid[x+1,y] >= 0 && originalGrid[x+1,y] < 8)
			{
				x += 1;
			}else if(y-1 >= 0 && grid[x,y-1] == d && originalGrid[x,y -1] >= 0 && (originalGrid[x,y -1] < 4 || (originalGrid[x,y -1] > 7 && originalGrid[x,y -1] < 12)))
			{
				y -= 1;
	...
	public void Find(ILabirint lab, IDraw drawLabirint)
	{
		originalGrid = lab.GetLab ();
		drawLab = drawLabirint;
		sizeLab = (int)Mathf.Sqrt (originalGrid.Length);
		Find ((int)sizeLab/2, (int)sizeLab/2, sizeLab - 1, sizeLab - 1);
		ShowWay ();
	}
	...
	void ShowWay()
	{
		...
		lineRender.SetVertexCount (path.Count);
